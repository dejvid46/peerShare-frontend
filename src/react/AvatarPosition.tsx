...
export default function AvatarPosition({ id, order, count }: AvatarProps) {
  ...
  if(order === 1){
    return <Avatar icon={id % 24} rotate={0} translateX={0}/>;
  }
  ...
  const circleSize = maxStep === step ? (count - 1) - steps[step-1] : steps[step] - steps[step-1];
  const rotate = 360/circleSize*(positionInCircle) + 270;
  const translateX = step*200;

  return (
    <Avatar icon={id % 24} rotate={rotate} translateX={translateX}/>
  );
}
